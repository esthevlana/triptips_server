...
router.get('/search-continent/:continent', async(req, res, next)=> {
    try {
        ...
        const searchedContinent = await Continent.findOne({continent:  {'$regex': continent , $options:'i'}})
        ...
        for (var i = 0; i < searchedContinent.countries.length; i++){
                 ...
                 const findArticles = await Article.find({countryName: searchedContinent.countries[i]})
